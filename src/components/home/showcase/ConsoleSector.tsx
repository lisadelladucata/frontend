...
export default function ConsoleSelector() {
  ...
  // Funzione per astrarre il contenuto di ogni tab e applicare la logica di allineamento
  const renderTabContent = (consoleName: string, bgColor: string) => (
    <div className={`p-3 pb-12 md:pb-20 ${bgColor}`}>
      <Container className="w-[98%] mx-auto">
        ...
        {/*
          Modifica chiave: Aggiunto 'items-stretch' per forzare l'altezza uniforme delle ProductCard
          (Funziona grazie a 'h-full' e 'flex flex-col' nella ProductCard)
        */}

        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 md:gap-8 **items-stretch**">
          {products?.data?.products?.length < 1 && (
            <p className="text-lg text-white">No {consoleName} found!</p>
          )}

          {products?.data?.products?.map((product: any) => (
            <ProductCard product={product} key={product._id} layout="photo" />
          ))}
        </div>
      </Container>
    </div>
          ...
          items={[
            {
              ...
              children: renderTabContent("Xbox", "bg-[#63b95d]"),
            },
            {
              key: "playstation",
              label: (
              ...
              children: renderTabContent("Playstation", "bg-[#1761bf]"),
            },
            {
              key: "nintendo",
              label: (
              ...
              children: renderTabContent("Nintendo", "bg-[#f34040]"),
            },
          ]}
        />
      </div>
